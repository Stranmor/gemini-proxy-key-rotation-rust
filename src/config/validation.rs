...
impl ConfigValidator {
    ...
    fn validate_groups(config: &AppConfig) -> Result<()> {
        ...
        for group in &config.groups {
            ...
            // Validate target URL
            debug!(
                "Validating target URL for group '{}': {}",
                group.name, group.target_url
            );
        ...
        debug!(
            "Validated {} groups with {} total keys",
            config.groups.len(),
            all_keys.len()
        );
        Ok(())
    ...
    fn validate_proxy_url(group_name: &str, proxy_url: &str) -> Result<()> {
        let url = Url::parse(proxy_url).map_err(|e| {
            AppError::config_validation(
                format!("Invalid proxy URL in group '{group_name}': {proxy_url} - {e}"),
                Some("proxy"),
            )
        })?;
        ...
        match url.scheme() {
            ...
            scheme => Err(AppError::config_validation(
                format!(
                    "Unsupported proxy scheme '{scheme}' in group '{group_name}'. Supported: http, https, socks5"
                ),
                Some("proxy"),
            )),
        }
