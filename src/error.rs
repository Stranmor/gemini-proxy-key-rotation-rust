...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        let body = Json(ErrorResponse { error: error_details });
...
mod tests {
    ...
    async fn check_response(
        ...
        let bytes = to_bytes(body, usize::MAX).await.expect("Failed to read response body");
        let body_json: Value = serde_json::from_slice(&bytes)
            .unwrap_or_else(|e| panic!("Response body is not valid JSON: {}. Body: {}", e, String::from_utf8_lossy(&bytes)));

        let error_obj = &body_json["error"];
        assert!(!error_obj.is_null(), "JSON 'error' field is missing");

        let error_type = error_obj["type"].as_str().expect("JSON 'error.type' field is not a string or missing");
        assert_eq!(error_type, expected_type, "Error type mismatch");

        let error_msg = error_obj["message"].as_str().expect("JSON 'error.message' field is not a string or missing");
        assert!(
            error_msg.contains(expected_message_substring),
            "Expected error message '{}' to contain '{}'", error_msg, expected_message_substring
        );

        if expect_details {
            assert!(!error_obj["details"].is_null(), "Expected 'error.details' field to exist but it was null or missing");
            assert!(error_obj["details"].is_string(), "Expected 'error.details' field to be a string");
        } else {
             assert!(error_obj["details"].is_null() || !error_obj["details"].is_string(), "Expected 'error.details' field to be null or non-existent/non-string");
        }
    }

    #[tokio::test]
    async fn test_into_response_config() {
        check_response(AppError::Config("Test config issue".to_string()), StatusCode::INTERNAL_SERVER_ERROR, "CONFIG_ERROR", "Internal server configuration error", false).await;
    }

    #[tokio::test]
    async fn test_into_response_io() {
        let io_error = io::Error::new(io::ErrorKind::NotFound, "File not found");
        check_response(AppError::Io(io_error), StatusCode::INTERNAL_SERVER_ERROR, "IO_ERROR", "Internal server error during IO operation", false).await;
    }

    #[tokio::test]
    async fn test_into_response_yaml() {
        let yaml_error: serde_yaml::Error = serde_yaml::from_str::<()>("invalid: yaml:").unwrap_err();
        check_response(AppError::YamlParsing(yaml_error), StatusCode::INTERNAL_SERVER_ERROR, "CONFIG_PARSE_ERROR", "Failed to parse configuration file", false).await;
    }

    #[tokio::test]
    async fn test_into_response_proxy_config_url_parse() {
        let url_error = url::Url::parse("::invalid url").unwrap_err();
        let proxy_error = ProxyConfigErrorData { url: "::invalid url".to_string(), kind: ProxyConfigErrorKind::UrlParse(url_error) };
        check_response(AppError::ProxyConfigError(proxy_error), StatusCode::INTERNAL_SERVER_ERROR, "PROXY_CONFIG_ERROR", "Internal server error related to proxy configuration", true).await; // Expect details (URL)
    }

     #[tokio::test]
     async fn test_into_response_proxy_config_unsupported_scheme() {
         let proxy_error = ProxyConfigErrorData { url: "ftp://bad".to_string(), kind: ProxyConfigErrorKind::UnsupportedScheme("ftp".to_string()) };
         check_response(AppError::ProxyConfigError(proxy_error), StatusCode::INTERNAL_SERVER_ERROR, "PROXY_CONFIG_ERROR", "Internal server error related to proxy configuration", true).await; // Expect details (URL)
     }

    #[tokio::test]
    async fn test_into_response_no_keys() {
        check_response(AppError::NoAvailableKeys, StatusCode::SERVICE_UNAVAILABLE, "NO_AVAILABLE_KEYS", "No available API keys", false).await;
    }
    ...
    async fn test_into_response_request_body_error() {
        check_response(AppError::RequestBodyError("Bad body".to_string()), StatusCode::BAD_REQUEST, "REQUEST_BODY_ERROR", "Failed to process request body", true).await;
    }

    #[tokio::test]
    async fn test_into_response_response_body_error() {
        check_response(AppError::ResponseBodyError("Bad response body".to_string()), StatusCode::BAD_GATEWAY, "RESPONSE_PROCESSING_ERROR", "Failed to process response", true).await;
    }

    #[tokio::test]
    // Removed ignore from this test
    async fn test_into_response_invalid_client_key() {
        check_response(AppError::InvalidClientApiKey, StatusCode::UNAUTHORIZED, "INVALID_API_KEY", "Invalid or unauthorized API key", false).await;
    }

    #[tokio::test]
    async fn test_into_response_internal() {
        check_response(AppError::Internal("Something went wrong".to_string()), StatusCode::INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR", "unexpected internal server error", false).await;
    }
     ...
     async fn test_into_response_http_client_build_error() {
         ...
         let build_error = builder_with_proxy.build().expect_err("Client build should fail with invalid proxy setup");

         check_response(
             // Manually construct the error variant now
             AppError::HttpClientBuildError{ source: build_error, proxy_url: Some(invalid_proxy_url.to_string())},
             StatusCode::INTERNAL_SERVER_ERROR,
     ...
     async fn test_into_response_reqwest_timeout() {
      ...
      async fn test_into_response_reqwest_generic() {
          ...
          let e = reqwest::get("http://invalid-url-that-does-not-exist-and-causes-error").await.unwrap_err();
           // Check based on the new logic: connection errors fall under "connect" message
           check_response(AppError::Reqwest(e), StatusCode::BAD_GATEWAY, "UPSTREAM_ERROR", "Could not connect to upstream service", true).await;
      }
