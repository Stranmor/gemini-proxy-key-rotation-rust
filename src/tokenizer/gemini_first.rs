...
impl GeminiFirstTokenizer {
    ...
    pub fn initialize(
        config: Option<GeminiFirstConfig>,
        ...
        info!(
            "Fast estimation threshold: {} chars",
            config.fast_estimation_threshold
        );
        ...
        if let Some(limit) = self.config.pre_check_limit {
            if text_length > self.config.fast_estimation_threshold {
                ...
                if estimated_tokens > limit {
                    debug!(
                        "Fast estimation: {} tokens > {} limit, rejecting",
                        estimated_tokens, limit
                    );
                    return TokenizationDecision::RejectTooLarge(estimated_tokens);
                } else {
                    debug!(
                        "Fast estimation: {} tokens < {} limit, sending directly",
                        estimated_tokens, limit
                    );
                    return TokenizationDecision::SendDirectly;
                }
    ...
    fn fast_estimate_tokens(&self, text: &str) -> usize {
        ...
        let unicode_ratio =
            text.chars().filter(|c| !c.is_ascii()).count() as f64 / char_count as f64;
        ...
        debug!(
            "Fast estimation: {} chars → {} tokens (ratio: {:.1})",
    ...
    pub fn count_tokens_post_response(
        &self,
        request_text: &str,
        response_text: &str,
    ) -> PostResponseTokens {
    ...
    pub fn get_info(&self) -> String {
        format!(
            ...
            self.config.enable_pre_check, self.config.use_fast_estimation
        )
    }
}
...
mod tests {
    ...
    fn test_post_response_counting() {
        ...
        assert_eq!(
            tokens.total_tokens,
            tokens.request_tokens + tokens.response_tokens
        );
    ...
    fn test_performance_on_large_text() {
        ...
        let large_text =
            "This is a comprehensive test document with various content types. ".repeat(10000);
