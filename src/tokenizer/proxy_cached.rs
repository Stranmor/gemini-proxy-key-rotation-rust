...
impl ProxyCachedTokenizer {
    ...
    pub async fn count_tokens(&self, text: &str) -> Result<usize, Box<dyn Error + Send + Sync>> {
        ...
        match self.get_google_token_count(text).await {
            Ok(count) => {
                ...
                info!("Cached new token count: {} for text length: {}", count, text.len());
                Ok(count)
            }
            Err(e) => {
    ...
    async fn get_google_token_count(&self, text: &str) -> Result<usize, Box<dyn Error + Send + Sync>> {
        let url = format!(
    ...
    pub async fn warm_cache(&self, common_texts: Vec<&str>) -> Result<(), Box<dyn Error + Send + Sync>> {
