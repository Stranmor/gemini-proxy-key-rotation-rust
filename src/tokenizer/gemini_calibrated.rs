...
impl GeminiCalibratedTokenizer {
    ...
    fn apply_calibration(&self, text: &str, base_count: usize) -> usize {
        ...
        let math_symbols = text.chars().filter(|c| {
            matches!(*c, '∑' | '∫' | '∂' | '∇' | '∞' | 'π' | 'α' | 'β' | 'γ' | 'δ' | '±' | '≤' | '≥' | '≠')
        }).count();
        if math_symbols > 5 {
        ...
        let code_indicators = text.matches('{').count() + text.matches('}').count() + 
                             ...
                             text.matches("if").count() + text.matches("return").count();
        ...
        match len {
            ...
            5..=8 => {
                ...
                if word.starts_with("un") || word.starts_with("re") || 
                   word.ends_with("ing") || word.ends_with("ed") || word.ends_with("ly") {
                    2
                } else {
                    1
                }
...
mod tests {
    ...
    async fn test_calibrated_performance() {
        ...
        assert!(duration.as_millis() < 100, "Calibrated tokenization should be fast");
    }
}