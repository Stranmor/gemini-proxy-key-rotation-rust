...
impl SmartParallelTokenizer {
    ...
    pub fn initialize(config: Option<SmartParallelConfig>) -> Result<(), Box<dyn Error + Send + Sync>> {
        let config = config.unwrap_or_default();
        
        info!("Initializing Smart Parallel Tokenizer");
        info!("Token limit: {}", config.token_limit);
        info!("Safe threshold: {} ({}%)", config.safe_threshold, 
            (config.safe_threshold as f64 / config.token_limit as f64 * 100.0) as u32);
        info!("Conservative estimate: {:.1} chars/token", config.chars_per_token_conservative);
    ...
    pub fn make_processing_decision(&self, text: &str) -> ProcessingDecision {
        ...
        let estimated_tokens = (char_count as f64 / self.config.chars_per_token_conservative).ceil() as usize;
        
        debug!("Text analysis: {} chars → ~{} tokens (conservative)", char_count, estimated_tokens);
    ...
    pub async fn process_text<F, Fut, T>(&self, text: &str, send_function: F) -> Result<(T, ProcessingResult), Box<dyn Error + Send + Sync>>
    where
        F: FnOnce(String) -> Fut,
        Fut: std::future::Future<Output = Result<T, Box<dyn Error + Send + Sync>>>,
    {
        ...
        match decision {
            ...
            ProcessingDecision::RejectImmediately { estimated_tokens } => {
                warn!("Rejecting request: estimated {} tokens > {} limit", 
                    estimated_tokens, self.config.token_limit);
                
                Err(format!("Request too large: estimated {} tokens exceeds limit of {}", 
                    estimated_tokens, self.config.token_limit).into())
            }
            ...
            ProcessingDecision::ParallelProcessing { estimated_tokens } => {
                ...
                if self.config.enable_parallel_sending {
                    self.process_parallel(text, estimated_tokens, send_function, start_time).await
                } else {
                    self.process_sequential(text, estimated_tokens, send_function, start_time).await
                }
            }
        }
    }
        ...
        match tokenization_result {
            Ok(actual_tokens) => {
                if actual_tokens > self.config.token_limit {
                    warn!("Token limit exceeded: {} > {}, but request already sent", 
                ...
                Ok((network_response, ProcessingResult {
        ...
        Fut: std::future::Future<Output = Result<T, Box<dyn Error + Send + Sync>>>,
    {
        ...
        // Проверяем лимит
        if actual_tokens > self.config.token_limit {
            return Err(format!("Request too large: {} tokens exceeds limit of {}", 
                actual_tokens, self.config.token_limit).into());
        }
    ...
    async fn count_tokens_with_timeout(&self, text: &str) -> Result<usize, Box<dyn Error + Send + Sync>> {
        ...
        match timeout(timeout_duration, tokenization_future).await {
            ...
            Err(_) => {
                ...
                let conservative_estimate = (text.len() as f64 / self.config.chars_per_token_conservative).ceil() as usize;
...
/// Обрабатывает текст с умной логикой
pub async fn process_text_smart<F, Fut, T>(text: &str, send_function: F) -> Result<(T, ProcessingResult), Box<dyn Error + Send + Sync>>
where
    F: FnOnce(String) -> Fut,
    Fut: std::future::Future<Output = Result<T, Box<dyn Error + Send + Sync>>>,
{
...
mod tests {
    ...
    async fn test_safety_guarantees() {
        ...
        let send_function = |_text: String| async move {
