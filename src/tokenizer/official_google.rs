...
"#;
    ...
    async fn verify_setup(&self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        match output {
            Ok(result) => {
                if result.status.success() {
                    ...
                    if stderr.contains("vertexai package not installed") {
                        Err(
                            "❌ Please install: pip install google-cloud-aiplatform[tokenization]"
                                .into(),
                        )
                    } else {
            ...
            Err(e) => Err(format!("❌ Python3 not found or not accessible: {e}").into()),
        }
    }
    ...
    pub fn count_tokens(
        ...
        if result["success"].as_bool().unwrap_or(false) {
            let token_count = result["total_tokens"]
                .as_u64()
...
pub fn count_official_google_tokens_for_model(
    text: &str,
    model: &str,
) -> Result<usize, Box<dyn Error + Send + Sync>> {
    let tokenizer = OFFICIAL_TOKENIZER
