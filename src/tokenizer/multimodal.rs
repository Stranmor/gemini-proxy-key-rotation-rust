...
type ProcessMessageResult =
    Result<(usize, usize, usize, Vec<ImageTokenInfo>), Box<dyn Error + Send + Sync>>;
...
impl MultimodalTokenizer {
    ...
    pub fn initialize(
        config: Option<MultimodalConfig>,
    ...
    pub fn count_tokens(
        &self,
        json_body: &Value,
    ) -> Result<TokenCount, Box<dyn Error + Send + Sync>> {
    ...
    fn count_image_tokens(
        &self,
        image_url: &Value,
    ) -> Result<(usize, ImageTokenInfo), Box<dyn Error + Send + Sync>> {
        let url = image_url
            .get("url")
            .and_then(|u| u.as_str())
        ...
        let base64_data = url.split(',').nth(1).ok_or("Invalid base64 image format")?;
        ...
        if base64_size > self.config.max_image_size {
            return Err(format!(
                ...
                base64_size, self.config.max_image_size
            )
            .into());
        }
...
pub fn count_multimodal_tokens(
    ...
    let tokenizer = MULTIMODAL_TOKENIZER.get().ok_or(
        "Multimodal tokenizer not initialized. Call MultimodalTokenizer::initialize() first.",
    )?;
...
mod tests {
    ...
    async fn test_multimodal_tokenizer_initialization() {
        ...
        assert!(
            result.is_ok(),
            "Multimodal tokenizer initialization failed: {result:?}"
        );
    }
    ...
    async fn test_text_only_message() {
        ...
        crate::tokenizer::GeminiTokenizer::initialize()
            .await
            .unwrap();
    ...
    async fn test_performance() {
        ...
        assert!(
            ...
            "Multimodal tokenization should be fast"
        );
    }
}
