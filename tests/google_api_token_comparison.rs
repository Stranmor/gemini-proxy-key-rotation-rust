...
async fn test_token_count_accuracy_vs_google_api() {
    ...
    for (i, text) in test_cases.iter().enumerate() {
        ...
        println!(
            "  Our count: {our_count} | Google count: {google_count} | Accuracy: {accuracy:.1}%"
        );
...
async fn test_multimodal_token_accuracy() {
    ...
    for (i, test_case) in test_cases.iter().enumerate() {
        ...
        // Получаем количество токенов от Google API
        let google_count =
            match get_google_multimodal_token_count(&client, &api_key, test_case).await {
                Ok(count) => count,
                Err(e) => {
...
async fn test_calibrate_tokenizer() {
    ...
    if !calibration_data.is_empty() {
        let avg_ratio: f64 = calibration_data
            .iter()
            .map(|(_, _, _, ratio)| ratio)
            .sum::<f64>()
            / calibration_data.len() as f64;
        ...
        if avg_ratio > 1.5 {
            println!(
                "🚨 RECOMMENDATION: Reduce token estimates by factor of {:.2}",
                1.0 / avg_ratio
            );
        } else if avg_ratio < 0.7 {
            println!(
                "🚨 RECOMMENDATION: Increase token estimates by factor of {:.2}",
                1.0 / avg_ratio
            );
        } else {
