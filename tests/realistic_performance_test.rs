...
async fn test_realistic_performance_comparison() {
    ...
    println!(
        "   Tokenization time: {:>8.2}ms",
        tokenization_time.as_millis()
    );
    ...
    println!(
        "   Network request:   {:>8.2}ms",
        network_time_direct.as_millis()
    );
    println!(
        "   Post-count time:   {:>8.2}ms",
        post_count_time.as_millis()
    );
    println!(
        "   TOTAL TIME:        {:>8.2}ms",
    ...
    println!(
        "  ✅ Eliminated tokenization: {}ms",
        tokenization_time.as_millis()
    );
    println!(
        "  ✅ Fast decision making:    {}ms",
        decision_time.as_millis()
    );
    println!(
        "  ✅ Quick post-counting:     {}ms",
        post_count_time.as_millis()
    );
    println!(
        "  🌐 Network time (same):     {}ms",
        network_time.as_millis()
    );

    println!("\n💡 Key Insights:");
    println!(
        "  • Network request time is the same (~{}ms)",
        network_time.as_millis()
    );
    println!(
        "  • We save {}ms by skipping pre-tokenization",
        tokenization_time.as_millis()
    );
    println!(
        "  • Post-response counting is {}x faster than pre-tokenization",
    ...
    assert!(
        time_saved > 10,
        "Should save at least 10ms, saved {time_saved}ms"
    );
...
async fn test_scalability_with_network() {
    ...
    for (size, description) in test_sizes {
        ...
        let time_saved =
            total_traditional.as_millis() as i64 - total_gemini_first.as_millis() as i64;
        let speedup = total_traditional.as_millis() as f64 / total_gemini_first.as_millis() as f64;

        println!(
            "{:<25} | {:>9}ms | {:>9}ms | {:>7}ms | {:>6.1}x",
            description,
            total_traditional.as_millis(),
            total_gemini_first.as_millis(),
            time_saved,
