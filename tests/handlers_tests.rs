...
use gemini_proxy::handlers::{
    base::ResponseHandler, invalid_api_key::InvalidApiKeyHandler, rate_limit::RateLimitHandler,
    success::SuccessHandler, terminal_error::TerminalErrorHandler, timeout::TimeoutHandler,
};
...
async fn test_invalid_api_key_handler() {
    ...
    let _body_bytes = Bytes::from("forbidden");
...
fn test_handler_response_types() {
    ...
    assert!(invalid_key
        .handle(&bad_request_response, &api_key_invalid_bytes, "key")
        .is_some());

    // Timeout handler should handle timeout codes
    let timeout_response = create_mock_response(StatusCode::GATEWAY_TIMEOUT);
    assert!(timeout
        .handle(&timeout_response, &body_bytes, "key")
        .is_some());
}
