...
async fn test_calibrated_vs_simple_performance() {
    ...
    println!(
        "Simple tokenizer: {} iterations in {:?} (avg: {:?})",
        iterations,
        simple_duration,
        simple_duration / iterations
    );
    println!(
        "Calibrated tokenizer: {} iterations in {:?} (avg: {:?})",
        iterations,
        calibrated_duration,
        calibrated_duration / iterations
    );
    ...
    assert!(
        overhead < 3.0,
        "Calibrated tokenizer is too slow: {overhead:.2}x overhead"
    );
