...
async fn test_ultimate_tokenizer_comparison() {
    ...
    let official_available =
        match tokenizer::official_google::OfficialGoogleTokenizer::initialize().await {
            Ok(_) => {
    ...
    let test_cases = vec![
        ...
        (
            "JSON data",
            r#"{"name": "John", "age": 30, "city": "New York", "hobbies": ["reading", "coding"]}"#,
        ),
    ];
    ...
    for (name, text) in test_cases {
        ...
        total_tests += 1;
        if simple_accuracy >= 95.0 {
            simple_accurate += 1;
        }
        if ml_accuracy >= 95.0 {
            ml_accurate += 1;
        }
        if official_accuracy >= 95.0 {
            official_accurate += 1;
        }
        if proxy_accuracy >= 95.0 {
            proxy_accurate += 1;
        }
...
async fn test_tokenizer_performance_comparison() {
    ...
    let official_available = tokenizer::official_google::OfficialGoogleTokenizer::initialize()
        .await
        .is_ok();
    ...
    if let Some(duration) = official_duration {
        println!(
            "  Official:      {:>8.2}ms avg ({:>6.2}ms total)",
