...
impl KeyManagerTrait for MockKeyManager {
    ...
    async fn get_key_states(
        &self,
    ) -> Result<HashMap<String, KeyState>, gemini_proxy::error::AppError> {
...
async fn test_key_health_monitor_recovery() {
    ...
    monitor
        .record_success(test_key, Duration::from_millis(100))
        .await;
...
async fn test_key_health_monitor_get_unhealthy_keys() {
    ...
    monitor
        .record_success("test-key-2", Duration::from_millis(100))
        .await;
...
async fn test_monitoring_system_with_key_activity() {
    ...
    key_health
        .record_success("test-key-1", Duration::from_millis(100))
        .await;
    key_health
        .record_success("test-key-2", Duration::from_millis(150))
        .await;
    ...
    assert!(
        stats.healthy_keys >= 2,
        "Should have at least 2 healthy keys"
    );
    assert!(
        stats.unhealthy_keys >= 1,
        "Should have at least 1 unhealthy key"
    );
...
async fn test_health_score_calculation() {
    ...
    assert!(
        (key_stat.health_score - 0.4).abs() < 0.1,
        "Health score should be approximately 0.4, got {}",
        key_stat.health_score
    );
}
...
mod monitoring_integration_tests {
    ...
    async fn test_monitoring_system_start() {
        ...
        if start_result.is_err() {
            // Таймаут - ожидаемо для бесконечного цикла
        } else {
            // Быстрое завершение - тоже нормально
        }
    }
    ...
    async fn test_key_health_monitor_background_checks() {
        ...
        let _stats = monitor.get_health_stats().await;
