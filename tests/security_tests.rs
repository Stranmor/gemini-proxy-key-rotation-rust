...
use gemini_proxy::security::{token_manager::TokenManager, SecurityMiddleware};
...
async fn test_security_middleware_rate_limiting() {
    ...
    let is_limited = security
        .is_rate_limited(&client_addr.ip().to_string())
        .await;
    ...
    for _ in 0..5 {
        security
            .record_failed_attempt(&client_addr.ip().to_string())
            .await;
    }

    // Теперь IP должен быть заблокирован
    let is_limited_after = security
        .is_rate_limited(&client_addr.ip().to_string())
        .await;
    assert!(
        is_limited_after,
...
async fn test_security_middleware_https_enforcement() {
    let security = SecurityMiddleware::new();
    let _client_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 8080);
...
async fn test_token_manager_session_tokens() {
    ...
    let session_token = token_manager
        .generate_session_token(client_ip.clone())
        .await;
    assert!(
        session_token.starts_with("st_"),
        "Session token should have correct prefix"
    );

    // Проверяем валидность токена
    let is_valid = token_manager
        .validate_token(&session_token, client_ip.clone())
        .await;
    assert!(is_valid, "Session token should be valid");

    // Проверяем мастер-токен
    let is_master_valid = token_manager
        .validate_token("master-token-123", client_ip.clone())
        .await;
    assert!(is_master_valid, "Master token should be valid");

    // Проверяем невалидный токен
    let is_invalid = token_manager
        .validate_token("invalid-token", client_ip)
        .await;
...
async fn test_token_manager_ip_validation() {
    ...
    let session_token = token_manager
        .generate_session_token(original_ip.clone())
        .await;

    // Проверяем с правильным IP
    let is_valid_correct_ip = token_manager
        .validate_token(&session_token, original_ip)
        .await;
    assert!(is_valid_correct_ip, "Token should be valid with correct IP");

    // Проверяем с другим IP
    let is_valid_different_ip = token_manager
        .validate_token(&session_token, different_ip)
        .await;
    assert!(
        !is_valid_different_ip,
        "Token should not be valid with different IP"
    );
}
...
async fn test_token_manager_revocation() {
    ...
    let is_valid_before = token_manager
        .validate_token(&session_token, client_ip.clone())
        .await;
    ...
    let is_valid_after = token_manager
        .validate_token(&session_token, client_ip)
        .await;
    assert!(
        !is_valid_after,
...
async fn test_token_manager_statistics() {
    ...
    let _token1 = token_manager
        .generate_session_token(Some("192.168.1.1".to_string()))
        .await;
    let _token2 = token_manager
        .generate_session_token(Some("192.168.1.2".to_string()))
        .await;
...
async fn test_token_manager_revoke_all_sessions() {
    ...
    assert!(
        token_manager
            .validate_token(&token1, Some("192.168.1.1".to_string()))
            .await
    );
    assert!(
        token_manager
            .validate_token(&token2, Some("192.168.1.2".to_string()))
            .await
    );
    ...
    assert!(
        token_manager
            .validate_token("master-token-123", Some("192.168.1.1".to_string()))
            .await
    );
}
