...
async fn test_smart_parallel_logic() {
    ...
    for (name, text, expected_decision) in test_cases {
        ...
        assert_eq!(
            decision_type, expected_decision,
...
async fn test_parallel_processing_performance() {
    ...
    match result {
        Ok((response, processing_result)) => {
            ...
            println!(
                "  Decision time:      {}ms",
                processing_result.decision_time_ms
            );
            println!(
            ...
            println!(
                "  Total time:         {}ms",
                processing_result.total_time_ms
            );
            println!("  Was parallel:       {}", processing_result.was_parallel);
            println!(
                "  Estimated tokens:   {}",
                processing_result.estimated_tokens
            );
            println!(
                "  Actual tokens:      {:?}",
                processing_result.actual_tokens
            );

            // Проверяем что параллельная обработка действительно быстрее
            assert!(
                processing_result.was_parallel,
                "Should use parallel processing"
            );
            assert!(
                processing_result.total_time_ms < 600,
                "Parallel processing should be fast, took {}ms",
                processing_result.total_time_ms
            );
            ...
            assert!(
                processing_result.total_time_ms <= expected_sequential_time,
...
async fn test_performance_vs_traditional() {
    ...
    let result = tokenizer::process_text_smart(&test_text, create_send_function())
        .await
        .unwrap();
    ...
    assert!(
        smart_total <= traditional_total + tolerance,
